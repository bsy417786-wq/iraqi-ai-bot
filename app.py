...
st.markdown(f"<p style='text-align:center; color:#94a3b8;'>نظام المبيعات لـ {STORE_NAME}</p>", unsafe_allow_html=True)
...
if prompt := st.chat_input("اكتب رسالتك هنا..."):
    ...
    sys_instruction = f"أنت '{EXPERT_NAME}' من شركة '{BRAND_NAME}'. بلهجة بغدادية، اطلب الاسم والرقم لتثبيت الحجز. إذا استلمتهم، قل حصراً: [تم تسجيل طلبك يا بطل]."

    try:
        # 1. استخراج البيانات (الزبدة)
        extract_res = requests.post(url, headers=headers, json={
            "model": "llama-3.1-8b-instant",
            "messages": [{"role": "system", "content": "Return ONLY JSON: {name, phone, order}"}, {"role": "user", "content": prompt}]
        }, timeout=7)
        ...
        if response.status_code == 200:
            ...
            # 3. الإرسال للإكسل (تأكد من وجود رقم هاتف في الرسالة)
            if re.search(r'07\d{8,9}', prompt):
                try:
                    ...
                    # نرسل المعلومات المستخرجة
                    send_to_excel(
                        clean_json.get('name', 'زبون جديد'),
                        clean_json.get('phone', '07xxxxxxx'),
                        clean_json.get('order', prompt)
                    )
                except:
                    # احتياط في حال فشل الاستخراج
